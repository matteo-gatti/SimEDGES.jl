...
"""
    ...
    h = ipergrafo in analisi
    metaV = treshold relativo ai vertici dell'ipergrafo
    metaE = treshold relativo ai nodi dell'ipergrafo
...
function bisect(h, metaV, metaE; opt=false)
    ...
    while left < right
        ...
        sres = simulate!(h, actV, actE, metaV, metaE; printme = true)
        if sres.actvs != nhv(h)
            left = location
        else
            right = location-1
